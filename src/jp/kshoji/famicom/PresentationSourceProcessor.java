...
/**
 * テキストを「text/0000.txt」から始まる連番で格納します。
 * 文字コードはUTF-8で、英数などを含めて、全て全角文字です。
 * 横32 x 縦28のテキストです。
 * 改行コードは「\r」「\n」「\r\n」どれでもOKです。
 * 
 * テキストの上に被せる画像を「overlay/0000.png」から始まる連番で格納します。
 * あんまり大きなデータだと成功しません。
 * 
 * このクラスのmainを呼びます。
 * cc65/make.shを実行します。
...
public class PresentationSourceProcessor {
	...
	private static void convertTextToPngImage() throws Exception {
		...
		for (int fileNumber = 0; fileNumber < new File("text").listFiles(new FileFilter() {
			...
			List<String> stringList = Utils.textfileToStringArray(new File(String.format("text/%04d.txt", fileNumber)));
			for (int y = 0; y < stringList.size(); y++) {
				String text = stringList.get(y);
				char[] charArray = text.toCharArray();
				for (int x = 0; x < charArray.length; x++) {
					if (charArray[x] >= '2' && charArray[x] <= '9') {
						// 2〜9倍角
						float fontSize = 8.f * (charArray[x] - '0') - 1.f;
						graphics2d.setFont(vlfont.deriveFont(Font.BOLD, fontSize));
						
						x++;
						graphics2d.drawString(new String(new char[] {charArray[x]}), x * 8, y * 8 + fontSize);
						continue;
					}
					
					int index = kutenTable.indexOf(charArray[x]);
					if (index < 0) {
					...
					if (index != 0) 
					graphics2d.drawImage(misaki.getSubimage((index % 94) * 8, index / 94 * 8, 8, 8), x * 8, y * 8, null);
				}
			}
			graphics2d.finalize();
			graphics2d.dispose();
	
			ImageIO.write(image, "png", new File(String.format("screen/%04d.png", fileNumber)));
		}
	}

	private static final boolean USE_RLE = true;

	static class BankInformation {
		int page;
		int bank;
	}
	
	/**
	 * 256x224のPNG画像をヘッダファイルとして書き出す
	 * 
	 * 戻り値
	 * ページ数
	 * 各ページのバンク番号
	 ...
	 * @throws Exception
	 */
	private static List<BankInformation> convertImageToCHeaderFile() throws Exception {
		int totalCharacters = 0;
		StringBuffer resultBuffer = new StringBuffer();
		int currentBank = 0;
		
		List<BankInformation> bankInformations = new ArrayList<BankInformation>();
		List<byte[]> characterList = new ArrayList<byte[]>();
		
		for (int fileNumber = 0; fileNumber < new File("screen").listFiles().length; fileNumber++) {
			BufferedImage src = ImageIO.read(new File(String.format("screen/%04d.png", fileNumber)));
			File overlayFile = new File(String.format("overlay/%04d.png", fileNumber));
			if (overlayFile.exists()) {
				...
				for (int y = 0; y < src.getHeight(); y++) {
					for (int x = 0; x < src.getWidth(); x++) {
						...
						if (y >= overlay.getHeight()) {
							continue;
						}
						
						src.setRGB(x, y, src.getRGB(x, y) + overlay.getRGB(x, y));
					}
				}
				ImageIO.write(src, "png", new File(String.format("processed/%04d.png", fileNumber)));
			}
			for (int y = 0; y < src.getHeight(); y++) {
				for (int x = 0; x < src.getWidth(); x++) {
					int rgb = src.getRGB(x, y);
					double gray = 0.299 * (rgb & 0xff) + 0.587 * ((rgb >> 8) & 0xff) + 0.114 * ((rgb >> 16) & 0xff);
					if (gray > 191) {
						rgb = 0xc0c0c0;
					} else if (gray > 127) {
						rgb = 0x808080;
			...
			resultBuffer.append(String.format("// ---- screen %04d\n", fileNumber));
			
			while (true) {
				try {
					totalCharacters = saveCharacterImage(totalCharacters, resultBuffer, characterList, src, currentBank);
					break;
			...
			bankInformation.bank = currentBank;
			bankInformation.page = totalCharacters < 256 ? 0 : 1;
			bankInformations.add(bankInformation);
		}
		
		// output c header file
		FileOutputStream fos = new FileOutputStream(new File(CSourceCodeDirectoryName + File.separator + "map_table.h"));
		fos.write(resultBuffer.toString().getBytes());
		fos.flush();
		fos.close();
	...
	// ページ間で "256" (バンク境界)をまたがないようにする(超えたら256をベースにしてリトライ)。
	private static int saveCharacterImage(int totalCharacters, StringBuffer resultBuffer, List<byte[]> characterList, BufferedImage src, int currentBank) throws IOException {
		int initialTotalCharacters = totalCharacters;
		new File(String.format("png/bank%02d", currentBank)).mkdirs();
		
		StringBuffer pageBuffer = new StringBuffer();
		pageBuffer.append(String.format("// bank : %d - %d\n", currentBank, totalCharacters>=256 ? 1 : 0));
		
		// RLE用
		int rleLength = 0;
		int rleData = -10;
		
		for (int y = 0; y < src.getHeight(); y += 8) {
			for (int x = 0; x < src.getWidth(); x += 8) {
				BufferedImage subimage = src.getSubimage(x, y, 8, 8);
				byte[] chrdata = Utils.bufferedImageToChrData(subimage);
				
				// 新しいキャラクタかを判定
				boolean matched = false;
				int matchedIndex = -1;
				for (int i = 0; i < characterList.size(); i++) {
					byte[] data = characterList.get(i);
					if (Utils.matches(chrdata, data)) {
						matched = true;
						matchedIndex = i;
						if (totalCharacters >=256) {
							matchedIndex += 256;
						}
					}
				}
				...
				if (!matched) {
					// 新しいキャラクタを登録
					matchedIndex = totalCharacters;
					totalCharacters++;
					
					if (USE_RLE) {
						if (totalCharacters == 254) {
							// 1ページに収まらなかった
							if (initialTotalCharacters == 0) {
								// ページ先頭だったのに収まらない → これ以上トライしてもムダ
								throw new RuntimeException();
							}
							throw new IndexOutOfBoundsException();
						} else if (totalCharacters == 510) {
							// バンク一杯まできた
							if (initialTotalCharacters == 256) {
								// ページ先頭だったのに収まらない → これ以上トライしてもムダ
								throw new RuntimeException();
							}
					...
					ImageIO.write(subimage, "png", new File(String.format("png/bank%02d/%04d.png", currentBank, matchedIndex)));
					characterList.add(chrdata);
				}
				
				// ファイルに追記
				if (USE_RLE) {
					if (totalCharacters >= 256 && matchedIndex > 256) {
						matchedIndex -= 256;
					}
					
					if (rleData == matchedIndex) {
						// 同じデータ
						rleLength++;
					} else {
						// 違うデータ
						...
						if (rleData >= 0) {
							if (rleLength >= 4) {
								// 長さが4以上ならRLEするほうが有利
								while (rleLength > 254) {
									pageBuffer.append("254,");
									pageBuffer.append("254,");
									pageBuffer.append(String.format("%d,", rleData));
									rleLength -= 254;
								}
	
								pageBuffer.append("254,");
								pageBuffer.append(String.format("%d,", rleLength + 1));
								pageBuffer.append(String.format("%d,", rleData));
							} else {
								// 長さが3以下なのでそのまま
						...
						// データをrleのバッファに追加
						rleData = matchedIndex;
						rleLength = 0;
					}
				} else {
					if (totalCharacters >= 256) {
						pageBuffer.append("" + (matchedIndex - 256) + ",");
					} else {
						pageBuffer.append("" + matchedIndex + ",");
					}
				}
			}
		...
		if (USE_RLE) {
			...
			if (rleLength >= 4) {
				...
				pageBuffer.append("254,");
				pageBuffer.append(String.format("%d,", rleLength));
				pageBuffer.append(String.format("%d,", rleData));
			} else {
				// 長さが3以下なのでそのまま
				for (int i = 0; i < rleLength; i++) {
			...
			pageBuffer.append("255,\n");
		}
		
		resultBuffer.append(pageBuffer);
		return totalCharacters;
	}

	/**
	 * PNG画像からCHRROMのデータに変換する。
	 * 
	 * @return
	 * @throws Exception
	 */
	private static int convertPngImageToChrromData() throws Exception {
		int chrromBankCount = 0;
		File[] bankDir = new File("png/").listFiles();
		for (File file : bankDir) {
			String dirName = file.getName();
			if (!dirName.startsWith("bank")) {
				continue;
			}
			chrromBankCount++;
			
			FileOutputStream fos = new FileOutputStream(new File(CSourceCodeDirectoryName + File.separator + dirName + ".chr"));
			for (int i = 0; i < 512; i++) {
				File pngFile = new File(String.format("png/" + dirName + "/%04d.png", i));
				BufferedImage src;
				if (pngFile.exists()) {
					src = ImageIO.read(pngFile);
				} else {
					src = new BufferedImage(8, 8, BufferedImage.TYPE_INT_RGB);
				}
				byte[] data = Utils.bufferedImageToChrData(src);
				fos.write(data);
			}
			fos.flush();
			fos.close();
		...
		return chrromBankCount;
	}
	
	private static void initializeResultDirectory() {
		Utils.cleanUpDirectory("png");
		Utils.cleanUpDirectory("processed");
		Utils.cleanUpDirectory("screen");
	}

	/**
	 * スライドのページに対応するバンク、ページの情報、総ページ数情報をslide.hに出力する。
	 * 
	 * @param bankInformations
	 * @throws FileNotFoundException
	 * @throws IOException
	 */
	private static void generateSlideInformation(List<BankInformation> bankInformations) throws FileNotFoundException, IOException {
		FileOutputStream fos = new FileOutputStream(new File(CSourceCodeDirectoryName + File.separator + "slide.h"));
		StringBuffer bankForSlide = new StringBuffer();
		StringBuffer pageForSlide = new StringBuffer();
		for (int i = 0; i < bankInformations.size(); i++) {
			BankInformation bankInformation = bankInformations.get(i);
			bankForSlide.append(bankInformation.bank);
			pageForSlide.append(bankInformation.page);

			if (i != bankInformations.size() - 1) {
				bankForSlide.append(",");
				pageForSlide.append(",");
			}
		}
		...
		"#define SLIDES_HEIGHT (28)\n" + //
				"#define SLIDES_MAX (" + bankInformations.size() + ")\n" + //
				"const unsigned char slides[] = {\n" + //
				"#include \"map_table.h\"\n" + //
				"};\n\n" + //
				"const unsigned char bank_for_slide[] = { " + bankForSlide.toString() + " };\n" + //
				"const unsigned char page_for_slide[] = { " + pageForSlide.toString() + " }; \n" //
		).getBytes());
		fos.flush();
		fos.close();
	}
	...
	/**
	 * キャラクタROMの情報をchrrom.sに出力する。
	 * 
	 * @param chrromBankCount
	 * @throws FileNotFoundException
	 * @throws IOException
	 */
	private static void generateChrromInformation(int chrromBankCount) throws FileNotFoundException, IOException {
		FileOutputStream fos = new FileOutputStream(new File(CSourceCodeDirectoryName + File.separator + "chrrom.s"));
		for (int i = 0; i < chrromBankCount; i++) {
			fos.write(String.format("\t.incbin \"bank%02d.chr\"\n", i).getBytes());
		}
		fos.flush();
		fos.close();
	}
	...
	public static void main(String[] notUsed) throws Exception {
		...
		System.out.println("converting text files into PNG screen images..");
		convertTextToPngImage();
		System.out.println("converting PNG screen images into 8x8 PNG character images..");
		List<BankInformation> bankInformations = convertImageToCHeaderFile();
		System.out.println("converting PNG character images into CHRROM files..");
		int chrromBankCount = convertPngImageToChrromData();
		
		System.out.println("generating header files..");
		generateChrromInformation(chrromBankCount);
		generateSlideInformation(bankInformations);
		
		System.out.println("all processes done successfully!");
	}
}
